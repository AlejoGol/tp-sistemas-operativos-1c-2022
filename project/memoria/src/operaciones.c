...
uint32_t escribir_dato(uint32_t direccion_fisica, uint32_t valor, uint32_t viene_de_swap) {
  uint32_t byte_inicio = obtener_byte_inicio(direccion_fisica, 1, viene_de_swap); // 1 para modificar bit_modif
...
uint32_t buscar_dato_en_memoria(uint32_t direccion_fisica, uint32_t viene_de_swap) {
  uint32_t byte_inicio = obtener_byte_inicio(direccion_fisica, 0, viene_de_swap); // 0 para no modificar bit_modif
...
uint32_t obtener_byte_inicio(uint32_t direccion_fisica, uint32_t bit_modificado, uint32_t viene_de_swap) {
  uint32_t marco_buscado = obtener_marco_dato(direccion_fisica);
  uint32_t offset_buscado = obtener_offset_dato(direccion_fisica);

  actualizar_bits(marco_buscado, bit_modificado, viene_de_swap);
...
void actualizar_bits(uint32_t numero_marco, uint32_t bit_modificado, uint32_t viene_de_swap) {
  if (viene_de_swap == 0) {
    return;
  }
